...
public:
    WindowTest() 
        : QMainWindow(),
        _view(new TreeView()),
        _model(new TreeViewModel(QStringList() << "First" << "Second"))
    {
    ...
    private slots:
        void slot_customContextMenuRequested(const QPoint &/*pos*/) {
            ...
            connect(action_add_category, SIGNAL(triggered()), this, SLOT(slot_add_category()));
            ...
            connect(action_append, SIGNAL(triggered()), this, SLOT(slot_appendElement()));
            QAction *action_removeAll = new QAction("remove all elements", this);
            connect(action_removeAll, SIGNAL(triggered()), this, SLOT(slot_removeAll()));
...
private:
    void populateTree() {
        ...
        root->appendChild(new TestItem(new ModelElem("First element after root", 1), root));
        _model->insertItem(0, new TestItem(new ModelElem("Next element added at idx 0", 1)));
    ...
    ModelElem *createModelElem() {
        static int i = 0;
        ModelElem *elem = new ModelElem(QString("new_elem_inside_%1").arg(i), i);
        ++i;
        return elem;
    }
