...
WdMenuBar::WdMenuBar(QWidget *parent)
    : QMenuBar(parent), _menus() {
}
...
WdMenu *WdMenuBar::getWdMenu(QString const &keyname,
    ...
    for (auto keyPath : menuPath) {
        ...
        if (menu) {
            ...
            if (keyExists(keyPath)) {
                qWarning() << "The key" << keyPath <<
                           "already exists in another level or is attributed to an action";
                return nullptr;
            }
            else {
...
WdMenu *WdMenuBar::insertWdMenu(QAction *before,
...
WdMenu *WdMenuBar::insertWdMenu(WdMenu *menu,
        QAction *before,
        QString const &keyname,
        QString const &text) {
    if (keyExists(keyname)) {
...
WdAction *WdMenuBar::addWdAction(QString const &keyname,
    ...
    WdMenu *currentMenu = getWdMenu(menuPath);
    if (currentMenu) {
        return currentMenu->addWdAction(keyname, text, receiver, slotName, shortcut);
    }
    return nullptr;
}
...
WdAction *WdMenuBar::addWdAction(QString const &keyname,
    ...
    WdAction *action = addWdAction(keyname, menuPath, text, receiver, slotName, shortcut);
    if (action) {
        action->setIcon(icon);
    }
    return action;
...
////////////////////// BEGIN INSERT /////////////////////////////
bool WdMenuBar::insertWdAction(WdMenu *menu,
        QAction *before,
        WdAction *action) {
...
WdAction *WdMenuBar::insertWdAction(WdMenu *menu,
        QAction *before,
        QString const &keyname,
        QString const &text) {
    if (keyExists(keyname)) {
