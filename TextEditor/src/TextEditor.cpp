...
: QPlainTextEdit(parent), _callTipAutoCompletion(new CallTip(this)) {
...
void TextEditor::setCurrentDocument(QTextDocument *doc) {
    ...
    QObject::connect(doc, SIGNAL(contentsChange(int, int, int)), this,
                     SLOT(slot_textChanged(int, int, int)));
    setDocument(doc);
}

void TextEditor::slot_textChanged(const int position, const int charsRemoved,
...
int TextEditor::positionFromLine(const int line, const int index) const {
    ...
    cursor.movePosition(QTextCursor::MoveOperation::Right,
                        QTextCursor::MoveMode::MoveAnchor, index);
...
void TextEditor::setFormat(const int position, const int length,
...
QTextCharFormat TextEditor::getFormat(const int position,
