...
public:
    ...
    */
    ...
    void setDocument(QTextDocument *document) {
        _textEditor->setCurrentDocument(document);
    }
    ...
    QString getText(const int position, const int length) const {
        return _textEditor->getText(position, length);
    }
    ...
    int positionFromLine(const int line, const int index) const {
        return _textEditor->positionFromLine(line, index);
    }
    void positionToLine(const int pos, int *line, int *index) {
        _textEditor->positionToLine(pos, line, index);
    }
    ...
    void selectText(const int position, const int length) {
        _textEditor->selectText(position, length);
    }
    void deleteText(const int position, const int length) {
        _textEditor->deleteText(position, length);
    }
    void insertText(const int position, QString const &text) {
        _textEditor->insertText(position, text);
    }
    void replaceText(const int position, const int length,
                     QString const &text) {
        _textEditor->replaceText(position, length, text);
    }

    void showCallTipAutoCompletion() {
        _textEditor->showCallTipAutoCompletion();
    }
    void hideCallTipAutoCompletion() {
        _textEditor->hideCallTipAutoCompletion();
    }
