...
int LineNumberSidebar::idealWidth() const
{
    ...
    if (lineNumber < 10000) return 3 + _fontMetricsWidth * 4;
    int result = 1;
    ...
    for (;;) {
        ...
        lineNumber /= 10000;
        result += 4;
        if (lineNumber < 10) return 3 + _fontMetricsWidth * result;
        if (lineNumber < 100) return 3 + _fontMetricsWidth * (result + 1);
        if (lineNumber < 1000) return 3 + _fontMetricsWidth * (result + 2);
        if (lineNumber < 10000) return 3 + _fontMetricsWidth * (result + 3);
    }
    return result;
}
...
void LineNumberSidebar::paintEvent(QPaintEvent *ev)
{
    ...
    while (block.isValid() && top <= ev->rect().bottom()) {
        if (block.isVisible() && bottom >= ev->rect().top()) {
            ...
            painter.drawText(0, top, width, fontMetricsHeight,
